...
RIGHT_FRACTION = 0.41  # widened for spacing
ROW_PAD_Y, HEADER_PAD_Y = 4, 6

COL_FRAC = [0.02, 0.12, 0.34, 0.55, 0.70, 0.85, 0.93]  # right UI: widened ALT space

TRAIL_MAX_POINTS = 1500
TRAIL_KEEP_SEC   = 86400
TRAIL_WIDTH      = 3  # Trail Size
MAX_HOP_NM       = 6.0
MAX_SAMPLE_GAP   = 14.0
MAX_SEG_PX       = 1200
MIN_MOVE_NM      = 0.07
...
DELTA_SIZE_PX = 16  # Delta Size
DELTA_LINE_W  = 2
DELTA_DOT_R   = 2

pygame.init()
pygame.display.set_caption("Lightning (Fixed Edition)")
...
SITE_NAME = "[REDCACTED]"
SITE_LAT, SITE_LON = 34.24921, -118.26855
R_NM = 3440.065
...
def draw_scene():
    ...
    for ac in acs:
        ...
        if not declutter:
            ...
            screen.blit(tag_font.render(cs,True,WHITE),(x+23,y-12))
        if trails_on:
            hx=ac.get('hex')
